...
EOF
    ...
    # Determine DNS servers based on local DNS configuration
    local dhcp_dns_servers="$DNS_SERVERS"
    if [[ "${LOCAL_DNS_ENABLED:-false}" == "true" ]]; then
        # If local DNS is enabled, use PXE server first, then external DNS as fallback
        dhcp_dns_servers="$PXE_SERVER_IP,${DNS_SERVERS}"
        echo -n "(with local DNS) "
    fi
...
subnet $SUBNET netmask $NETMASK {
    ...
    option domain-name "${DOMAIN_NAME:-local}";
